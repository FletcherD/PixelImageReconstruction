...
class ScaleEffectivenessVisualizer:
    ...
    def create_scaled_images(self, 
        ...
        Args:
            ...
            scale_range: (min_scale, max_scale) for both axes
    ...
    def run_inference_on_scaled_images(self, 
        ...
        Args:
            scaled_images: List of (scale_x, scale_y, image_path) tuples
        ...
        for true_scale_x, true_scale_y, image_path in scaled_images:
            try:
                # Run inference
                result = self.inference.predict_single(image_path)
                
                # Add ground truth information
                result['true_scale_x'] = true_scale_x
                result['true_scale_y'] = true_scale_y
                result['image_path'] = image_path
                
                # Calculate errors
                result['error_scale_x'] = result['scale_x'] - true_scale_x
                result['error_scale_y'] = result['scale_y'] - true_scale_y
                result['abs_error_scale_x'] = abs(result['error_scale_x'])
                result['abs_error_scale_y'] = abs(result['error_scale_y'])
                ...
                results.append({
                    'true_scale_x': true_scale_x,
                    'true_scale_y': true_scale_y,
    ...
    def create_visualization(self, 
                           ...
                           original_image_path: str):
        """
        Create a comprehensive visualization of the scale detection effectiveness.
        ...
        true_scales_x = np.array([r['true_scale_x'] for r in valid_results])
        true_scales_y = np.array([r['true_scale_y'] for r in valid_results])
        pred_scales_x = np.array([r['scale_x'] for r in valid_results])
        pred_scales_y = np.array([r['scale_y'] for r in valid_results])
        errors_x = np.array([r['error_scale_x'] for r in valid_results])
        errors_y = np.array([r['error_scale_y'] for r in valid_results])
        
        # Get unique scale values for grid
        unique_scales_x = np.unique(true_scales_x)
        unique_scales_y = np.unique(true_scales_y)
        
        # Create grid for heatmaps
        grid_shape = (len(unique_scales_y), len(unique_scales_x))
        ...
        for result in valid_results:
            i = np.where(unique_scales_y == result['true_scale_y'])[0][0]
            j = np.where(unique_scales_x == result['true_scale_x'])[0][0]
            
            pred_x_grid[i, j] = result['scale_x']
            pred_y_grid[i, j] = result['scale_y']
            error_x_grid[i, j] = result['error_scale_x']
            error_y_grid[i, j] = result['error_scale_y']
        ...
        ax4.set_title('Scale X Error\n(Predicted - True)', fontsize=12)
        ax4.set_xlabel('True Scale X')
        ax4.set_ylabel('True Scale Y')
        ...
        ax5.set_title('Scale Y Error\n(Predicted - True)', fontsize=12)
        ax5.set_xlabel('True Scale X')
        ax5.set_ylabel('True Scale Y')
        plt.colorbar(im5, ax=ax5, shrink=0.8)
        
        # Scatter plots: Predicted vs True
        ax6 = plt.subplot(3, 4, 6)
        ax6.scatter(true_scales_x, pred_scales_x, alpha=0.7, c='blue', s=50)
        min_scale = min(true_scales_x.min(), pred_scales_x.min())
        max_scale = max(true_scales_x.max(), pred_scales_x.max())
        ax6.plot([min_scale, max_scale], [min_scale, max_scale], 'r--', alpha=0.8)
        ax6.set_xlabel('True Scale X')
        ax6.set_ylabel('Predicted Scale X')
        ax6.set_title('Scale X: Predicted vs True')
        ax6.grid(True, alpha=0.3)
        
        ax7 = plt.subplot(3, 4, 7)
        ax7.scatter(true_scales_y, pred_scales_y, alpha=0.7, c='green', s=50)
        min_scale = min(true_scales_y.min(), pred_scales_y.min())
        max_scale = max(true_scales_y.max(), pred_scales_y.max())
        ax7.plot([min_scale, max_scale], [min_scale, max_scale], 'r--', alpha=0.8)
        ax7.set_xlabel('True Scale Y')
        ax7.set_ylabel('Predicted Scale Y')
        ax7.set_title('Scale Y: Predicted vs True')
        ...
        ax8.set_xlabel('Scale X Error')
        ax8.set_ylabel('Frequency')
        ax8.set_title('Scale X Error Distribution')
        ax8.grid(True, alpha=0.3)
        
        ax9 = plt.subplot(3, 4, 9)
        ax9.hist(errors_y, bins=20, alpha=0.7, color='green', edgecolor='black')
        ax9.axvline(0, color='red', linestyle='--', alpha=0.8)
        ax9.set_xlabel('Scale Y Error')
        ax9.set_ylabel('Frequency')
        ax9.set_title('Scale Y Error Distribution')
        ax9.grid(True, alpha=0.3)
        
        # Statistics text
        ax10 = plt.subplot(3, 4, 10)
        stats_text = f"""Scale Detection Statistics:

Scale X:
  Mean Error: {errors_x.mean():.4f} ± {errors_x.std():.4f}
  MAE: {np.abs(errors_x).mean():.4f}
  RMSE: {np.sqrt((errors_x**2).mean()):.4f}
  R²: {np.corrcoef(true_scales_x, pred_scales_x)[0,1]**2:.4f}

Scale Y:
  Mean Error: {errors_y.mean():.4f} ± {errors_y.std():.4f}
  MAE: {np.abs(errors_y).mean():.4f}
  RMSE: {np.sqrt((errors_y**2).mean()):.4f}
  R²: {np.corrcoef(true_scales_y, pred_scales_y)[0,1]**2:.4f}

Dataset:
  Scale Range: [{unique_scales_x.min():.1f}, {unique_scales_x.max():.1f}]
  Grid Size: {len(unique_scales_x)} × {len(unique_scales_y)}
  Valid Samples: {len(valid_results)}/{len(results)}"""
        ...
        # Error vs scale magnitude
        ax11 = plt.subplot(3, 4, 11)
        scale_magnitude = np.sqrt(true_scales_x**2 + true_scales_y**2)
        error_magnitude = np.sqrt(errors_x**2 + errors_y**2)
        ax11.scatter(scale_magnitude, error_magnitude, alpha=0.7, s=50)
        ax11.set_xlabel('True Scale Magnitude')
        ax11.set_ylabel('Error Magnitude')
        ax11.set_title('Error vs Scale Magnitude')
        ax11.grid(True, alpha=0.3)
        
        # 2D error visualization
        ax12 = plt.subplot(3, 4, 12)
        ax12.quiver(true_scales_x, true_scales_y, errors_x, errors_y, 
                   angles='xy', scale_units='xy', scale=1, alpha=0.7, width=0.003)
        ax12.set_xlabel('True Scale X')
        ax12.set_ylabel('True Scale Y')
    ...
    def run_analysis(self, 
                    ...
                    cleanup: bool = True):
        """
        Run the complete scale effectiveness analysis.
        ...
        """
        print(f"Starting scale effectiveness analysis for {image_path}")
        print(f"Scale range: {scale_range}, Steps: {num_steps}")
        
        # Create scaled images
        scaled_images = self.create_scaled_images(
        ...
        # Run inference
        results = self.run_inference_on_scaled_images(scaled_images)
        ...
        valid_results = [r for r in results if 'error' not in r]
        if valid_results:
            errors_x = [r['error_scale_x'] for r in valid_results]
            errors_y = [r['error_scale_y'] for r in valid_results]
            
            print(f"\n=== Analysis Complete ===")
            print(f"Processed {len(valid_results)}/{len(results)} images successfully")
            print(f"Scale X - MAE: {np.mean(np.abs(errors_x)):.4f}, RMSE: {np.sqrt(np.mean([e**2 for e in errors_x])):.4f}")
            print(f"Scale Y - MAE: {np.mean(np.abs(errors_y)):.4f}, RMSE: {np.sqrt(np.mean([e**2 for e in errors_y])):.4f}")
            print(f"Visualization saved to: {output_path}")


def main():
    parser = argparse.ArgumentParser(description="Visualize scale detection model effectiveness")
    
    parser.add_argument("--image", type=str, required=True,
                       help="Input image path")
    parser.add_argument("--model_path", type=str, required=True,
                       help="Path to trained scale detection model")
    parser.add_argument("--output", type=str, default="scale_effectiveness_visualization.png",
    ...
    parser.add_argument("--num_steps", type=int, default=9,
                       help="Number of scale steps in each direction")
    
    # System parameters
    parser.add_argument("--input_size", type=int, default=128,
                       help="Model input size")
    parser.add_argument("--temp_dir", type=str,
                       help="Temporary directory for scaled images")
